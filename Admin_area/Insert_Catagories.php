...
if (isset($_POST['insert_category'])) {
  ...
  // tmp image
  $tmp_category_image = $_FILES['category_image']['tmp_name'];




  $select_query = "Select * from `catagories` where Category_tittle='$category_title' and Category_Image='$category_image'";

  $select_result = mysqli_query($conn, $select_query);
  $number = mysqli_num_rows($select_result);
  if ($category_title == "" or $category_image == "") {
...
<form action="" method="post" class="mt-2"  enctype="multipart/form-data">
  ...
  <div class="input-group m-3 w-90">
    <input type="file" class="form-control" name="category_image" id="category_image" placeholder="Insert Catagories Image">
  </div>
